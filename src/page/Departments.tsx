...
export default function Departments({ departments, employees, onAdd, onEdit, onDelete }: DepartmentsProps) {
  ...
  return (
    <div>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <button className="btn-add" onClick={openAdd}><MdAdd size={18} /> Add Department</button>
      </div>
          ...
          {departments.map(d => (
            <div key={d.id} className="dept-card">
              ...
              <div className="dept-card-actions">
                <button className="dept-action-btn edit" onClick={() => openEdit(d)} title="Edit"><MdEdit size={16} /></button>
                <button className="dept-action-btn delete" onClick={() => onDelete(d.id)} title="Delete"><MdDelete size={16} /></button>
              </div>
            </div>
          ))}
        </div>
