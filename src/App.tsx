...
import Attendance, { autoTag } from './page/Attendance'
...
import type { AttendanceRecord, Employee, Department, LeaveRecord, DailyAttendance } from './types'
...
function mapAttendance(a: { emp_id: string; date: string; check_in: string; check_out: string; status: 'Present' | 'Half Day' | 'Leave'; work_tag: 'overtime' | 'earlycome' | 'earlyout' | null }): DailyAttendance {
  return {
    emp_id:    a.emp_id,
    date:      a.date,
    check_in:  a.check_in  || '',
...
export default function App() {
  ...
  const [pendingUploads, setPendingUploads] = useState<DailyAttendance[]>([])
  ...
  const loadAll = useCallback(async () => {
    ...
    setApiError('')
    try {
      const [emps, depts, atts, lvs] = await Promise.all([
        employeeApi.getAll(),
  ...
  const handleUpdateAttendance = (_records: DailyAttendance[]) => {
    // Intentionally does NOT update global attendance state.
    // The Attendance page holds its own local editing state.
    // Dashboard / Report / LeaveList are unaffected.
  }

  /* Called by Attendance page after successful bulkUpsert */
  const handleAttendanceSaved = async (savedRecords: DailyAttendance[]) => {
    ...
    // Clear pending uploads since they have been saved!
    setPendingUploads([])
  }
  ...
  const handleDeleteAttendance = async (fromDate: string, toDate: string) => {
    try {
      ...
      alert(`Deleted ${res.deleted_count} record(s).`)
  ...
  const handleUpload = async (csvRecords: AttendanceRecord[], targetDate: string) => {
    ...
    // Build DailyAttendance — all records go under the selected target date
    const toUpsert: DailyAttendance[] = []
    csvRecords.forEach(r => {
      const emp = employees.find(e => e.name.toLowerCase() === r.emp_name.toLowerCase())
      const key = emp ? emp.emp_id : r.emp_name
      toUpsert.push({
        emp_id:    key,
        date:      uploadDate,
    ...
    // Set as pending uploads and navigate to attendance page for review
    setPendingUploads(toUpsert)
